...
//
  var templates = {
    ...
    user_nav: Hogan.compile('<a href="#" class="icon-sprite icon-user dropdown-toggle" data-toggle="dropdown"></a><ul class="dropdown-menu"><li><a href="#user-info" data-toggle="modal"><i class="icon-user"></i>查看账号详情</a></li><li class="divider"></li><li><a href="https://twitter.com/{{screen_name}}" target="_blank"><i class="icon-share-alt"></i>在 Twitter 上查看个人资料</a></li></ul>'),
    modal_header: Hogan.compile('<h3>截止到 {{created_at_relative}} 的账号详情<span class="download-date muted"></span></h3>'),
    ...
    modal_payload_details: Hogan.compile('<p>{{tweets}} <span class="footer-label muted">条推文</span></p>'),
    modal_account_details: Hogan.compile('<p>#{{id}} <span class="footer-label muted">用户 ID</span></p><p class="truncated">{{created_at_relative}} <span class="footer-label muted">加入</span></p>'),
    ...
    singular_tweet_count: Hogan.compile('{{count}} 条推文'),
    plural_tweet_count: Hogan.compile('{{count}} 条推文')
  };
//
  ...
  exports.init = function () {
    ...
    .bind('invalidSearch', function (e) {
      e.preventDefault();
      $(this)
        .attr('data-original-title', '你的请求必须包括至少两个字符。')
        .tooltip('show');
    });
  ...
  (function(exports) {
    exports.base = {
      init: function () {
        $('.navbar-search .search-query').attr('placeholder', '搜索全部推文');
